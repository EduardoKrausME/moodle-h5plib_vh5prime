...
interface H5PFrameworkInterface {
    ...
    public function fetchExternalData(
        $url, $data = null, $blocking = true, $stream = null, $fullData = false, $headers = [], $files = [], $method = 'POST'
    );
...
class H5PValidator {
    ...
    public function isValidPackage($skipContent = false, $upgradeOnly = false) {
        ...
        if (strtolower(substr($tmpPath, -3)) !== 'h5p') {
            $this->h5pF->setErrorMessage(
                $this->h5pF->t('The file you uploaded is not a valid HTML5 Package (It does not have the .h5p file extension)'),
        ...
        if ($zip->open($tmpPath) !== true) {
            $this->h5pF->setErrorMessage(
                $this->h5pF->t('The file you uploaded is not a valid HTML5 Package (We are unable to unzip it)'), 'unable-to-unzip'
            );
            unlink($tmpPath);
            return false;
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                $this->h5pF->setErrorMessage(
                    $this->h5pF->t(
                        'One of the files inside the package exceeds the maximum file size allowed. (%file %used > %max)', [
                            '%file' => $fileStat['name'], '%used' => ($fileStat['size'] / 1048576) .
                                ' MB', '%max' => ($this->h5pC->maxFileSize / 1048576) . ' MB',
                        ]
                    ), 'file-size-too-large'
                );
                $valid = false;
            ...
            } else if ($fileName === 'h5p.json') {
                $mainH5pExists = true;
            } else if ($fileName === 'content/content.json') {
                $contentExists = true;
            } else if (substr($fileName, 0, 8) === 'content/') {
            ...
            } else if ($canInstall && strpos($fileName, '/') !== false) {
        ...
        if ($skipContent === false) {
            ...
            if (!$mainH5pExists) {
                ...
                if ($mainH5pData === null) {
                    ...
                    $valid = false; // Validation error when parsing JSON
                } else if (!$this->isValidH5pData($mainH5pData, 'h5p.json', $this->h5pRequired, $this->h5pOptional)) {
                    $this->h5pF->setErrorMessage(
                        $this->h5pF->t('The main h5p.json file is not valid'), 'invalid-h5p-json-file'
                    ); // Is this message a bit redundant?
                    $valid = false;
                }
            }
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (!$fileStream) {
                ...
                $this->h5pF->setErrorMessage(
                    $this->h5pF->t('Unable to read file from the package: %fileName', ['%fileName' => $fileName]),
        ...
        if ($canInstall) {
            ...
            foreach ($files as $file) {
                ...
                if ($libraryH5PData['machineName'] !== $file && H5PCore::libraryToFolderName($libraryH5PData) !== $file) {
                    $this->h5pF->setErrorMessage(
                        $this->h5pF->t(
                            'Library directory name must match machineName or machineName-majorVersion.minorVersion (from library.json). (Directory: %directoryName , machineName: %machineName, majorVersion: %majorVersion, minorVersion: %minorVersion)',
                            [
                                '%directoryName' => $file,
                ...
                if ($skipContent || !$mainDependency || !$this->h5pF->libraryHasUpgrade([
                    ...
                    foreach ($missingLibraries as $libString => $library) {
                        if (!empty($mainDependency) && $library['machineName'] === $mainDependency['machineName']) {
                            $this->h5pF->setErrorMessage(
                                $this->h5pF->t('Missing main library @library', ['@library' => $libString]), 'missing-main-library'
                            );
                        } else {
                            $this->h5pF->setErrorMessage(
                                $this->h5pF->t('Missing required library @library', ['@library' => $libString]),
    ...
    private function getWhitelistRegExp($isLibrary) {
        $whitelist =
            $this->h5pF->getWhitelist($isLibrary, H5PCore::$defaultContentWhitelist, H5PCore::$defaultLibraryWhitelistExtras);
    ...
    public function getLibraryData($file, $filePath, $tmpDir) {
        ...
        if ($h5pData === false) {
            $this->h5pF->setErrorMessage(
                $this->h5pF->t('Could not find library.json file with valid json format for library %name', ['%name' => $file]),
        ...
        if (file_exists($semanticsPath)) {
            ...
            if ($semantics === false) {
                $this->h5pF->setErrorMessage(
                    $this->h5pF->t('Invalid semantics.json file has been included in the library %name', ['%name' => $file]),
        ...
        if (is_dir($languagePath)) {
            ...
            foreach ($languageFiles as $languageFile) {
                ...
                if (preg_match('/^(-?[a-z]+){1,7}\.json$/i', $languageFile) === 0) {
                    $this->h5pF->setErrorMessage(
                        $this->h5pF->t(
                            'Invalid language file %file in library %library', ['%file' => $languageFile, '%library' => $file]
    ...
    private function isExistingFiles($files, $tmpDir, $library) {
        foreach ($files as $file) {
            ...
            if (!file_exists($tmpDir . '/' . $library . '/' . $path)) {
                $this->h5pF->setErrorMessage(
                    $this->h5pF->t('The file "%file" is missing from library: "%name"', ['%file' => $path, '%name' => $library]),
    ...
    private function isValidRequirement($h5pData, $requirement, $library_name, $property_name) {
        ...
        if (is_string($requirement)) {
            ...
            $this->h5pF->setErrorMessage(
                $this->h5pF->t(
                    "Can't read the property %property in %library", ['%property' => $property_name, '%library' => $library_name]
                )
            );
            $valid = false;
        }
    ...
    private function isValidH5pDataOptions($selected, $allowed, $library_name) {
        ...
        foreach ($selected as $value) {
            if (!in_array($value, $allowed)) {
                $this->h5pF->setErrorMessage(
                    $this->h5pF->t('Illegal option %option in %library', ['%option' => $value, '%library' => $library_name]),
...
class H5PStorage {
    ...
    private function saveLibraries() {
        ...
        foreach ($this->h5pC->librariesJsonData as $libString => &$library) {
            ...
            $existingLibrary =
                $this->h5pC->loadLibrary($library['machineName'], $library['majorVersion'], $library['minorVersion']);
        ...
        if ($newOnes && $oldOnes) {
            if ($newOnes === 1) {
                if ($oldOnes === 1) {
                    // Singular Singular
                    $message = $this->h5pF->t(
                        'Added %new new H5P library and updated %old old one.', ['%new' => $newOnes, '%old' => $oldOnes]
                    );
                } else {
                    // Singular Plural
                    $message = $this->h5pF->t(
                        'Added %new new H5P library and updated %old old ones.', ['%new' => $newOnes, '%old' => $oldOnes]
                    );
                }
            } else {
                // Plural
                if ($oldOnes === 1) {
                    // Plural Singular
                    $message = $this->h5pF->t(
                        'Added %new new H5P libraries and updated %old old one.', ['%new' => $newOnes, '%old' => $oldOnes]
                    );
                } else {
                    // Plural Plural
                    $message = $this->h5pF->t(
                        'Added %new new H5P libraries and updated %old old ones.', ['%new' => $newOnes, '%old' => $oldOnes]
                    );
                }
            }
        } else if ($newOnes) {
...
class H5PExport {
    ...
    public function createExportFile($content) {
        ...
        foreach (
            ['authors', 'source', 'license', 'licenseVersion', 'licenseExtras', 'yearFrom', 'yearTo', 'changes', 'authorComments', 'defaultLanguage']
            as $field
        ) {
...
if (strpos($_SERVER['SCRIPT_FILENAME'], "edit.php") > 1) {

    if (file_exists("{$CFG->dirroot}/h5p/h5plib/v127/joubel/core/h5p.classes.php")) {
        core::$scripts = [
            "../../../v127/joubel/core/js/jquery.js",
            "../../../v127/joubel/core/js/h5p.js",
            "../../../v127/joubel/core/js/h5p-event-dispatcher.js",
            "../../../v127/joubel/core/js/h5p-x-api-event.js",
            "../../../v127/joubel/core/js/h5p-x-api.js",
            "../../../v127/joubel/core/js/h5p-content-type.js",
            "../../../v127/joubel/core/js/h5p-confirmation-dialog.js",
            "../../../v127/joubel/core/js/h5p-action-bar.js",
            "../../../v127/joubel/core/js/request-queue.js",
            "../../../v127/joubel/core/js/h5p-tooltip.js",
        ];
        core::$styles = [
            "../../../v127/joubel/core/styles/h5p.css",
            "../../../v127/joubel/core/styles/h5p-confirmation-dialog.css",
            "../../../v127/joubel/core/styles/h5p-core-button.css",
            "../../../v127/joubel/core/styles/h5p-tooltip.css",
            "../../../v127/joubel/core/styles/h5p-table.css",
        ];
    } else if (file_exists("{$CFG->dirroot}/h5p/h5plib/v128/joubel/core/h5p.classes.php")) {
        core::$scripts = [
            "../../../v128/joubel/core/js/jquery.js",
            "../../../v128/joubel/core/js/h5p.js",
            "../../../v128/joubel/core/js/h5p-event-dispatcher.js",
            "../../../v128/joubel/core/js/h5p-x-api-event.js",
            "../../../v128/joubel/core/js/h5p-x-api.js",
            "../../../v128/joubel/core/js/h5p-content-type.js",
            "../../../v128/joubel/core/js/h5p-confirmation-dialog.js",
            "../../../v128/joubel/core/js/h5p-action-bar.js",
            "../../../v128/joubel/core/js/request-queue.js",
            "../../../v128/joubel/core/js/h5p-tooltip.js",
        ];
        core::$styles = [
            "../../../v128/joubel/core/styles/h5p-fonts.css",
            "../../../v128/joubel/core/styles/h5p.css",
            "../../../v128/joubel/core/styles/h5p-confirmation-dialog.css",
            "../../../v128/joubel/core/styles/h5p-core-button.css",
            "../../../v128/joubel/core/styles/h5p-theme.css",
            "../../../v128/joubel/core/styles/h5p-theme-variables.css",
            "../../../v128/joubel/core/styles/h5p-tooltip.css",
            "../../../v128/joubel/core/styles/h5p-table.css",
        ];
    }
}
...
class H5PCore {
    ...
    public function getDependenciesFiles($dependencies, $prefix = '') {
        ...
        foreach ($dependencies as $dependency) {
            ...
            $dependency['version'] =
                "?ver={$dependency['majorVersion']}.{$dependency['minorVersion']}.{$dependency['patchVersion']}";
    ...
    public function findLibraryDependencies(&$dependencies, $library, $nextWeight = 1, $editor = false) {
        foreach (['dynamic', 'preloaded', 'editor'] as $type) {
            ...
            foreach ($library[$property] as $dependency) {
                ...
                $dependencyLibrary =
                    $this->loadLibrary($dependency['machineName'], $dependency['majorVersion'], $dependency['minorVersion']);
                if ($dependencyLibrary) {
                    ...
                    $nextWeight =
                        $this->findLibraryDependencies($dependencies, $dependencyLibrary, $nextWeight, $type === 'editor');
    ...
    public static function libraryToFolderName($library) {
        ...
        return "{$name}-{$library['majorVersion']}.{$library['minorVersion']}" .
            ($includePatchVersion ? ".{$library['patchVersion']}" : '');
    }
    ...
    public function getUpgrades($library, $versions) {
        ...
        foreach ($versions as $upgrade) {
            if ($upgrade->major_version > $library->major_version ||
                $upgrade->major_version === $library->major_version && $upgrade->minor_version > $library->minor_version) {
    ...
    public function getLibrariesInstalled() {
        ...
        foreach ($libs as $libName => $library) {
            foreach ($library as $libVersion) {
                $librariesInstalled[$libName . ' ' . $libVersion->major_version . '.' . $libVersion->minor_version] =
    ...
    public function fetchLibrariesMetadata($fetchingDisabled = false, $onlyRegister = false) {
        ...
        if (empty($uuid)) {
            ...
            if (!$registration) {
                $errorMessage =
                    $this->h5pF->t('Site could not be registered with the hub. Please contact your site administrator.');
    ...
    private function setDisplayOptionOverrides($option_name, $permission, $id, &$value) {
        ...
        if ($behaviour == H5PDisplayOptionBehaviour::NEVER_SHOW) {
            $value = false;
        } else if ($behaviour == H5PDisplayOptionBehaviour::ALWAYS_SHOW) {
            // If always show or permissions say so, force show
            $value = true;
        } else if ($behaviour == H5PDisplayOptionBehaviour::CONTROLLED_BY_PERMISSIONS) {
    ...
    public function getDisplayOptionsForView($disable, $id) {
        ...
        if ($this->h5pF->getOption(self::DISPLAY_OPTION_FRAME, true) == false) {
            ...
            $this->setDisplayOptionOverrides(
                self::DISPLAY_OPTION_DOWNLOAD, H5PPermission::DOWNLOAD_H5P, $id, $display_options[self::DISPLAY_OPTION_DOWNLOAD]
            );
            $this->setDisplayOptionOverrides(
                self::DISPLAY_OPTION_EMBED, H5PPermission::EMBED_H5P, $id, $display_options[self::DISPLAY_OPTION_EMBED]
            );
    ...
    public function getLibraryId($library, $libString = null) {
        ...
        if (!isset($libraryIdMap[$libString])) {
            $libraryIdMap[$libString] =
                $this->h5pF->getLibraryId($library['machineName'], $library['majorVersion'], $library['minorVersion']);
        }
    ...
    public function checkSetupErrorMessage() {
        ...
        if ($php_version[0] < 5 || ($php_version[0] === 5 && $php_version[1] < 2)) {
            $setup->errors[] = $this->h5pF->t(
                'Your PHP version is outdated. H5P requires version 5.2 to function properly. Version 5.6 or later is recommended.'
            );
            $setup->disable_hub = true;
        }
        ...
        if (!$this->fs->hasWriteAccess()) {
            $setup->errors[] = $this->h5pF->t(
                'A problem with the server write access was detected. Please make sure that your server can write to your data folder.'
            );
            $setup->disable_hub = true;
        }
        ...
        if ($max_upload_size > $max_post_size) {
            $setup->errors[] =
                $this->h5pF->t(
                    'Your PHP max upload size is bigger than your max post size. This is known to cause issues in some installations.'
                );
        }
    ...
    public function getLocalization() {
        return [
            ...
            'keywordsDescription' => $this->h5pF->t(
                'You can add multiple keywords separated by commas. Press "Enter" or "Add" to confirm keywords'
            ),
            'altText' => $this->h5pF->t('Alt text'),
            'reviewMessage' => $this->h5pF->t('Please review the info below before you share'),
            'subContentWarning' => $this->h5pF->t(
                'Sub-content (images, questions etc.) will be shared under :license unless otherwise specified in the authoring tool'
            ),
            ...
            'registrationFailedDescription' => $this->h5pF->t(
                'We were not able to create an account at this point. Something went wrong. Try again later.'
            ),
            ...
            'ageDescription' => $this->h5pF->t(
                'The target audience of this content. Possible input formats separated by commas: "1,34-45,-50,59-".'
            ),
            'invalidAge' => $this->h5pF->t(
                'Invalid input format for Typical age. Possible input formats separated by commas: "1, 34-45, -50, -59-".'
            ),
            'contactPersonDescription' => $this->h5pF->t(
            ...
            'emailAddressDescription' => $this->h5pF->t(
                'The email address will be used by H5P to reach out to the publisher in case of any issues with the content or in case the publisher needs to recover their account. It will not be published or shared with any third parties'
            ),
            'copyrightWarning' => $this->h5pF->t(
                'Copyrighted material cannot be shared in the H5P Content Hub. If the content is licensed with a OER friendly license like Creative Commons, please choose the appropriate license. If not this content cannot be shared.'
            ),
    ...
    public function hubRetrieveContent($id) {
        ...
        if (empty($response['data'])) {
            throw new Exception(
                $this->h5pF->t('Unable to authorize with the H5P Hub. Please check your Hub registration and connection.')
            );
        }
     ...
     * @param int $content_hub_id For updating existing content
     * @return stdClass
     */
    ...
    public function hubAccountInfo() {
        ...
        if (empty($siteUuid) && !empty($secret)) {
            $this->h5pF->setErrorMessage(
                $this->h5pF->t(
                    'H5P Hub secret is set without a site uuid. This may be fixed by restoring the site uuid or removing the hub secret and registering a new account with the content hub.'
                )
            );
        ...
        if ($accountInfo['status'] === 401) {
            ...
            $this->h5pF->setErrorMessage(
                $this->h5pF->t(
                    'Hub account authentication info is invalid. This may be fixed by an admin by restoring the hub secret or register a new account with the content hub.'
                )
            );
            return false;
        }
...
class H5PContentValidator {
    ...
    public function validateText(&$text, $semantics) {
        ...
        if (isset($semantics->tags)) {
            ...
            if (in_array('table', $tags)) {
                $tags =
                    array_merge($tags, ['tr', 'td', 'th', 'colgroup', 'col', 'thead', 'tbody', 'tfoot', 'figure', 'figcaption']);
            }
            ...
            if (isset($semantics->tags) && in_array('table', $semantics->tags)) {
                ...
                $stylePatterns[] =
                    '/^border: *[0-9.]+(em|px|%|) *(none|solid|dotted|dashed|double|groove|ridge|inset|outset) *(#[a-f0-9]{3}[a-f0-9]{3}?|rgba?\([0-9, ]+\)|hsla?\([0-9,.% ]+\)) *;?$/i';
        ...
        if (!in_array($value->library, $libraryNames)) {
            ...
            foreach ($libraryNames as $semanticsLibrary) {
                ...
                if ($machineName === $semanticsMachineName) {
                    ...
                    $message = $this->h5pF->t(
                        'The version of the H5P library %machineName used in this content is not valid. Content contains %contentLibrary, but it should be %semanticsLibrary.',
                        [
    ...
    private function filter_xss(
        $string, $allowed_tags = ['a', 'em', 'strong', 'cite', 'blockquote', 'code', 'ul', 'ol', 'li', 'dl', 'dt', 'dd'],
        $allowedStyles = false
    ) {
        if (strlen($string) == 0) {
            return $string;
    ...
    private function _filter_xss_split($m, $store = false) {
        ...
        $attr2 = implode(
            ' ', $this->_filter_xss_attributes(
            $attrList, (in_array($elem, self::$allowed_styleable_tags) ? $this->allowedStyles : false)
        )
        );
